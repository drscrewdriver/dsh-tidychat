...
> | --- | --- | --- | --- | --- |
> | **0.3.1（compat/0.1.5 线）** | **0.1.5-alpha.1+** | `installSection` | ✅ 正常（待 0.1.5 实测确认） | ✅ 可用（「接管官方消息轨」开关控制官方轨；0.1.5 TurnNavigator 硬编码于 ChatView，隐藏选择器待实测） |
...
| 📍 消息轨（Adaptive Navigation Rail） | 聊天区边缘的全局导航，可贴**左缘或右缘**（右缘整体镜像：强调三角指左、摘要卡从左侧弹出）。固定高度 Canvas minimap，任意轮次全局映射；鱼眼悬停、拖动预览、点击跳转、当前轮次高亮。两种显示样式：**横线** / **圆点**；另有独立**外圈**开关（当前轮与悬停轮外描一圈强调色）。配色可自适应，或用调色盘自定义（HEX/RGB 输入 + 透明度）。**更早历史未加载时，轨道顶部出现箭头提示带，悬停说明当前覆盖范围，点击即加载更早记录**。**DSH 0.1.2+ 需打开「接管官方消息轨」开关**才会接管（否则会与官方右缘轨并存） |
...
**消息轨（Canvas minimap）**：可贴左缘或右缘，样式可选横线 / 圆点，外圈可独立开关。悬停显示该轮摘要，点击跳转。下图为在无官方右缘 TurnNavigator 的旧版 DSH 上的运行效果（v0.2.10 起在 DSH 0.1.2+ 打开「接管官方消息轨」开关后同样可用）。
